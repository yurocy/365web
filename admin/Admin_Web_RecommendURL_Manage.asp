...
strFileName="Admin_Web_RecommendURL_Manage.asp?select_Query="&str_select&"&Query_text="&str_text
...
if ID<>"" then
	if Action="Del" then
		conn.execute "Delete From WebSiteRecommend_URL Where ID=" & CLng(ID)
	end if
end if
%>
<html>
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="border">
  <tr class="topbg">
    <td height="22" colspan="2" align="center"><strong>推 荐 栏 管 理</strong></td>
  </tr>
  <tr class="tdbg">
    <td width="70" height="30"><strong>管理导航：</strong></td>
    <td height="30"> <a href="Admin_Web_RecommendURL_Manage.asp?Action=list">网址管理</a> | <a href="Admin_Web_RecommendURL_Manage.asp?Action=Add">添加网站</a></td>
  </tr>
</table>
<br>
<%
...
sub main()
	sql="select * from WebSiteRecommend_URL "
...
<form method="post" name="AddLink" action="Admin_Web_RecommendURL_Manage.asp">
  <tr>
    <td width="76" align="right"><b>查询:</b>&nbsp;&nbsp;</td>
    <td><select size="1" name="select_Query">
        <option selected value="请选择">请选择</option>
        <option <%if str_select="Site_Title" then response.write "selected" end if%> value="Site_Title">网站名称</option>
...
%>
    <tr class="tdbg" onMouseOut="this.style.backgroundColor=''" onMouseOver="this.style.backgroundColor='#BFDFFF'">
      ...
      <td align="center">
<%
      response.write "<a href='Admin_Web_RecommendURL_Manage.asp?Action=Modify&ID=" & rs("ID") & "'>修改</a> | "
      response.write "<a href='Admin_Web_RecommendURL_Manage.asp?Action=Del&ID=" & rs("ID") & "' onclick='return ConfirmDel();'>删除</a>"
	  %>
...
sub Add()
%>
<form method="post" name="AddLink" onSubmit="return Check()" action="Admin_Web_RecommendURL_Manage.asp">
  <table border="0" cellpadding="2" cellspacing="1" align="center" width="100%" class="border">
    ...
    <tr class="tdbg">
      <td width="85" height="25" valign="middle"  align="right"><strong>是否推荐:</strong></td>
      <td height="25"><input type="checkbox" name="Recommend" value="1"></td>
    </tr>
    <tr class="tdbg"> 
...
sub Modify()
	...
	sqlLink="select * from WebSiteRecommend_URL where ID=" & ID
...
<form method="post" name="AddLink" onSubmit="return Check()" action="Admin_Web_RecommendURL_Manage.asp">
  <table border="0" cellpadding="2" cellspacing="1" align="center" width="100%" class="border">
    ...
    <tr class="tdbg">
      <td width="300" height="25" valign="middle"  align="right"><strong>是否推荐:</strong></td>
      <td height="25"><input type="checkbox" name="Recommend" value="1" <% if rsLink("Is_Recommend")=1 then response.write "checked" end if%>></td>
    </tr>
    <tr class="tdbg"> 
...
<%
sub SaveAdd()
	dim WebName,WebUrl,WebColor,Recommend
	...
	if FoundErr<>True then
		dim sqlLink,rsLink
		sqlLink="select top 1 * from WebSiteRecommend_URL where Site_Title='" & dvHtmlEncode(WebName) & "'"
		set rsLink=Server.CreateObject("Adodb.RecordSet")
		rsLink.open sqlLink,conn,1,3
		if not (rsLink.bof and rsLink.eof) then
			...
			set rsLink=nothing
			call CloseConn()
			Response.Redirect "Admin_Web_RecommendURL_Manage.asp?Action=Add"
		end if
		rsLink.close
		set rsLink=nothing
	end if
...
sub SaveModify()
	...
	if rsLink.bof and rsLink.eof then
		...
		set rsLink=nothing
		call CloseConn()
		Response.Redirect "Admin_Web_RecommendURL_Manage.asp"
	end if
	rsLink.close
	set rsLink=nothing
end sub
