...
sub Admin_ShowClass_Option(ShowType,CurrentID)
	if ShowType=0 then
		...
		response.write ">无(一级栏目)</option>"
	    response.write "<option value='-1'"
		response.write ">无(附加栏目)</option>"
...
if ShowType=3 then
    dim rsa
    strTemp=""
    sqlClass="Select * From MenuClass where ClassType='文档' order by RootID,OrderID"
...
sub Admin_ShowPath(RootName)
	...
	if ManageType="MyArticle" then
		response.write "<font color=red>" & AdminName & "</font> 的文档"
	else
		if keyword="" then
			response.write "所有文档"
		else
			select case strField
				case "Title"
					response.Write "标题中含有 <font color=red>"&keyword&"</font> 的文档"
				case "Content"
					response.Write "内容含有 <font color=red>"&keyword&"</font> 的文档"
				case "Author"
					response.Write "作者姓名中含有 <font color=red>"&keyword&"</font> 的文档"
				case "Editor"
					response.write "编辑姓名中含有 <font color=red>" & keyword & "</font> 的文档"
				case else
					response.Write "标题中含有 <font color=red>"&keyword&"</font> 的文档"
			end select
		end if
...
sub Admin_ShowSearchForm(Action,ShowType)
	...
	if ShowType=1 then
		...
		response.write "<input type='submit' name='Submit' class='button' value='搜索' style='border-style: solid; border-width: 1;background-color: #419010;'>"
		response.write "<br>高级搜索"
	elseif Showtype=2 then
		response.write "<select name='Field' size='1'>"
    	response.write "<option value='Title' selected>文档标题</option>"
	    response.write "<option value='Content'>文档内容</option>"
    	response.write "<option value='Author'>文档作者</option>"
		...
		response.write "<input type='text' name='keyword' class='Input'  size='20' value='关键字' maxlength='50' onFocus='this.select();'>"
		response.write "&nbsp;&nbsp;<input type='submit' name='Submit' class='button' value='搜索'>"
	end if
